...
class DirectoryPasswordEncoder
{
    ...
    {
        $uid   = $this->setUniqueEncryptionId();
    ...
    {
        $str = mcrypt_decrypt( MCRYPT_DES, $this->getUniqueEncryptionId(), $password, MCRYPT_MODE_ECB, 7 );
        $pad = ord( $str[( $len = strlen( $str ) ) - 1] );
        $decodedPassword = substr( $str, 0, strlen( $str ) - $pad );
        return $decodedPassword;
    }
    
    private function setUniqueEncryptionId(SESSION $session)
    {
    	
    	return $session->set('directoryEncryptionId', $this->gen_uuid());
    }
    
    private function getUniqueEncryptionId(SESSION $session)
    {
